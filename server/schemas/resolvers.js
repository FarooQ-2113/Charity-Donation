...
const resolvers = {
  ...
  Query: {
    ...
    charity: async (parent, { charityId }) => {
      return charity && charity.findOne({ _id: charityId });
    },

    // GET all Charities
    charities: async () => {
      return charity && charity.find().populate("categories");
    },
  ...
  Mutation: {
    ...
    saveCharity: async (parent, { charityId }, context) => {
      ...
      const charity = await charity && charity.findOne({ _id: charityId });

      // Saving all the categories from the charity found above in variable
      const categoryIds = await charity && charity.categories;
